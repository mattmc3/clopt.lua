...
function OptSet:parse(args, fallback)
	...
	while i <= #args do
		...
		if arg:sub(1, 2) == "--" then
			...
			-- No support for long options in this version
			return "unsupported long option: " .. arg
				...
				elseif cfg.has_arg then
					...
					if j < #group then
						...
						if val:sub(1,1) == "=" then
							val = val:sub(2)
						elseif #val > 0 then
							-- If there are extra chars after the option, that's an error
							return "missing argument for -" .. ch
						end
						j = #group
					elseif i <= #args then
						val = args[i]
					...
					if err then return err end
					break -- argument-consuming options must be last in group
				else
